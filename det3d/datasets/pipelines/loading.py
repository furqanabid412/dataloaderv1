...
class LoadPointCloudFromFile(object):
    ...
    def multi_sweeps(self,res,info):
        ...
        if nsweeps > 0:
            assert (nsweeps - 1) == len(info["sweeps"]), "nsweeps {} should equal to list length {}.".format(nsweeps, len(
                info["sweeps"]))
    ...
    def __call__(self, res, info):
        ...
        if self.type == "NuScenesDataset":
            ...
            # sweep_times_list = [np.zeros((points.shape[0], 1))]
            #
            # assert (nsweeps - 1) == len(info["sweeps"]), "nsweeps {} should equal to list length {}.".format(nsweeps, len(info["sweeps"]))
            #
            # for i in np.random.choice(len(info["sweeps"]), nsweeps - 1, replace=False):
            #     sweep = info["sweeps"][i]
            #     points_sweep, times_sweep = read_sweep(sweep, painted=res["painted"]) # use_seg NotImplemented
            ...
            points,times = self.multi_sweeps(res,info)
...
@PIPELINES.register_module
class LoadLidarSegLabels(object):
    def __init__(self, doAugmentation=True ,dataset='NUSCENES', **kwargs):
    ...
    def __call__(self, res, info):
        if self.dataset=='NUSCENES':
            if self.doAugmentation:
             res["lidar"]["lidarseg"] = np.expand_dims(load_bin_file(Path(info["ref_lidarseg"]["filename"]), type='lidarseg'),axis=1)
             #     res["lidar"]["combined"] = np.concatenate([points, lidarseg_labels], axis=1)
            # points = res["lidar"]["combined"]
            # if self.train:
            #     lidarseg_labels = np.expand_dims(load_bin_file(Path(info["ref_lidarseg"]["filename"]), type='lidarseg'),axis=1)
            #     res["lidar"]["combined"] = np.concatenate([points, lidarseg_labels], axis=1)
            # else:
            #     lidarseg_labels=np.full((len(points[:,0]),1),-1,dtype=np.uint8)
            #     res["lidar"]["combined"] = np.concatenate([points, lidarseg_labels], axis=1)
        return res,info
